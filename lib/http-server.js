...
server.on('request', (req, res) => {
    ...
    if(req.method === 'GET') {
        ...
        if(!splitPath[1]) {
            res.end('How can I help you today? Try entering a path, either fact or greeting.')
        }
        else if(splitPath[1] === 'fact'){
        ...
        res.statusCode = 404;
        res.end(`http method ${req.method} is not supported: we cannot ${req.method} the ${pathname}`);
    }
});
